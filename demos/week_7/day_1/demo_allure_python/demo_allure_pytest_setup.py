...
@allure.epic("Live coding challenge")
@allure.story("User should be able to log in with valid credentials")
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("User can log in with valid credentials")
def test_valid_credentials_successful_login():
    new_user = User(1, "John User", "john@example.com", "my_password_123")
    assert new_user.login("my_password_123")

@allure.epic("Live coding challenge")
@allure.story("User shouldn't be able to log in with invalid credentials")
@allure.severity(allure.severity_level.CRITICAL)
@allure.title("User is prevented from logging in when using invalid credentials")
def test_invalid_credentials_unsuccessful_login():
    new_user = User(2, "Jane User", "jane@example.com", "12345")
    assert not new_user.login("1")

@allure.epic("Live coding challenge")
@allure.story("User can log out after signing in.")
@allure.severity(allure.severity_level.NORMAL)
@allure.title("User can successfully log out after logging in.")
def test_logout():
    new_user = User(3, "Sam User", "sam@example.com", "cat")
    assert new_user.login("cat")
    new_user.logout()
    assert not new_user.is_online()
